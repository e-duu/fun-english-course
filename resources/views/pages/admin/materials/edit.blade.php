...
@section('content')
  <form action="{{ route('material.update', $data->id) }}" method="POST" class="px-4 py-3 bg-white rounded-lg shadow-md dark:bg-gray-800" enctype="multipart/form-data">
    ...
    <label class="block text-sm mt-4">
      ...
      <input type="text" name="title" value="{{ $data->title }}" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray rounded-md border-gray-400" placeholder="Lesson 1 Greetings - Presentation"/>
      @error('name')
    ...
    <label class="block text-sm mt-4">
      ...
      <input type="text" name="content" value="{{ $data->content }}" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray rounded-md border-gray-400" placeholder="https://docs.google.com/presentation"/>
      @error('content')
        <div class="mt-2" style="color: rgb(255, 35, 35);">
          <i class="fas fa-dot-circle"></i> {{ $message }}
        </div>
    ...
    <div class="mt-4 text-sm">
      ...
      <div class="mt-2">
        <label class="inline-flex items-center text-gray-600 dark:text-white">
          <input type="radio" class="text-blue-600 form-radio focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:focus:shadow-outline-gray" name="is_accessible_by_student" value="1" {{ ($data->is_accessible_by_student == 1 ? 'checked' : '') }} />
          <span class="ml-2">Yes</span>
        </label>
        <label class="inline-flex items-center ml-6 text-gray-600 dark:text-white">
          <input type="radio" class="text-blue-600 form-radio focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:focus:shadow-outline-gray" name="is_accessible_by_student" value="0" {{ ($data->is_accessible_by_student == 0 ? 'checked' : '') }} />
