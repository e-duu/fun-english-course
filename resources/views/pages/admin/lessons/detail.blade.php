...
<div class="flex-col sm:flex-row">
  <form class="flex-row justify-between relative">
    <button formaction="{{ route('material.create', $data->id) }}" class="my-1 sm:my-0 w-full sm:w-auto px-5 py-1 bg-[blueviolet] rounded-md font-semibold text-white">Add Material</button>
    
    <button formaction="{{ route('exercise.create', $data->id) }}" class="my-1 sm:my-0 w-full sm:w-auto px-5 py-1 bg-[blueviolet] rounded-md font-semibold text-white sm:mx-2">Add Exercise</button>
    
    <button formaction="{{ route('downloadable.create', $data->id) }}" class="my-1 sm:my-0 w-full sm:w-auto px-5 py-1 bg-[blueviolet] rounded-md font-semibold text-white">Add Downloadable File</button>
      ...
      <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-darker">
        ...
        @foreach ($data->downloadables as $downloadable)
          <tr class="text-gray-700 dark:text-gray-400">
            ...
            <td class="px-4 py-3">
              <div class="flex exercises-center space-x-4 text-sm">
                <a href="{{ route('downloadable.show', $downloadable->id) }}" class="flex-col text-center px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray">
                  <i class="fas fa-eye"></i>
                  <p>Detail</p>
                </a>
                <a href="{{ route('downloadable.edit', $downloadable->id) }}" class="flex-col text-center px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray">
                  <i class=" fas fa-edit"></i>
                  <p>Edit</p>
                </a>
