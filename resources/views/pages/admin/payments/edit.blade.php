...
@section('content')
  <form id="payments" action="{{ route('payment.store') }}" method="POST" class="px-4 py-3 bg-white rounded-lg shadow-md dark:bg-gray-800" enctype="multipart/form-data">
        ...
        <label class="block mt-4 text-sm">
          ...
          <select @change="setPrograms(programs_id)" v-model="programs_id" name="program_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray rounded-md border-gray-400 -600 dark:bg-gray-700 form-select focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:focus:shadow-outline-gray">
            @foreach ($programs as $program)
              <option value="{{ $program->id }}" {{ $program->name == $payments->programs->name ? 'selected' : null }}>{{ $program->name }}</option>
            @endforeach
          </select>
          @error('program_id')
            <div class="mt-1 text-sm text-[red]">
    ...
    <label class="block text-sm" style="margin-top: 20px">
      ...
      <input value="{{ $payments->amount }}" type="number" name="amount" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray rounded-md border-gray-400" placeholder="3"/>
      @error('amount')
        <div class="mt-1 text-sm text-[red]">
          <i class="fas fa-dot-circle text-xs"></i> {{ $message }}
        </div>
    ...
    <label class="block text-sm mt-4">
      ...
      <input type="file" name="evidence" class="border w-full mt-1 text-sm rounded-md border-gray-400 py-1 px-2 dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray" value="{{ old('evidence') }}" />
      @error('evidence')
