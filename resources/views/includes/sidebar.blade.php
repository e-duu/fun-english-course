<div class="flex-col p-10 bg-white h-full overflow-y-auto hidden sm:block">
    ...
    @foreach ($lessons as $lesson)
      @if ($lesson->exercises->count() || $lesson->materials->count() != null || $lesson->downloadables->count() != null)
        ...
        @foreach ($lesson->downloadables as $downloadable)
          <a href="{{ route('downloadable', $downloadable->id) }}" class="flex justify-between items-center space-x-2 py-4 px-3 hover:bg-blue-600 hover:text-white transition-all duration-200 rounded-md mt-3 {{ (request()->is('downloadable/' . $downloadable->id )) ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800' }}">
